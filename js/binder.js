...
(function () {
  ...
  function renderTabs() {
    ...
    const currentIssue = current.find((p) => p && !p.placeholder);
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      if (issue) btn.addEventListener("click", () => {
        ...
        goToSpread(firstSpreadOfIssue(B.issues.indexOf(issue)), true);
        renderTabs();
      });
  ...
  function renderSpread() {
    ...
    const flatIdx = flatIndexOfSpread(B.cur);
    const total = actualPages.length;
    const currentActual = (B.spreads[B.cur] || []).find((p) => p && !p.placeholder);
    const currentPageNumber = currentActual ? actualPages.findIndex((p) => samePage(p, currentActual)) + 1 : total;
    pageCountEl.textContent = total ? "Page " + M.clamp(currentPageNumber, 1, total) + " / " + total : "Page 0 / 0";

    const curPage = a && !a.placeholder ? a : (b && !b.placeholder ? b : null);
    if (curPage) {
  ...
  async function goToSpread(target, viaTab) {
    ...
    try {
      ...
      await animateFlip(dir, target, () => {
        B.cur = target;
        const targetIssuePage = [...(B.spreads[target] || [])].reverse().find((p) => p && !p.placeholder);
        if (targetIssuePage) B.selectedIssueId = targetIssuePage.issue.id;
        renderSpread();
      }, viaTab);
    } catch (err) {
